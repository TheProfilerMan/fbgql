...
#
#   ./run.sh                       # scrape PAGE (default ZainSudan)
...
# It will mint cookies (opens a browser) ONLY if cookies are missing/expired.
#
# Override anything via env, e.g.:
#   PAGE=ZainSudan POSTS=20 PROFILE=default ENGINE=threads COOKIES=cookies.json \
#   OUT=out/result.json ./run.sh
#
# NOTE: this is for hosts with a display. In containers / Apify you inject cookies
# instead — the browser login cannot run headless reliably.
set -euo pipefail
cd "$(dirname "$0")"
...
PAGE="${POSITIONAL[0]:-${PAGE:-ZainSudan}}"
...
if [ "$CMD" = "doctor" ]; then
  args=(doctor --cookies "$COOKIES" --page "$PAGE")
